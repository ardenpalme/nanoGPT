...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.c_fc    = nn.Linear(config.n_embd, 4 * config.head_size*config.n_head, bias=config.bias)
        self.gelu    = nn.GELU()
        self.c_proj  = nn.Linear(4 * config.head_size*config.n_head, config.n_embd, bias=config.bias)
...
@dataclass
class GPTConfig:
    #I believe block size is how much context?
    block_size: int = 96
